...
2. **Minimal DOM use.** Touch the Gorgias DOM as little as possible — only when there is no alternative. Ticket ID comes from the URL path (`/app/views/{viewId}/{ticketId}` — the ticket ID is the last segment; verified against a live account), not from page markup. Expected DOM interactions: anchor-probing to dock the panel (with floating fallback) and navigation-detection fallbacks. Any new DOM read must be justified, isolated in the shell, and driven by config-served selectors so fixes deploy without an extension release.
...
| `POST /v1/tickets/{id}/drafts/stream` | MVP | Same pipeline, streamed as SSE (`ticket` / `progress` / `delta` / `done` / `insufficient` / `error`). The `ticket` event fires as soon as the Gorgias fetch completes (customer, subject, language, message count) so the panel shows a header while the model warms up. `progress` (added 2026-08-05) narrates the pipeline's own steps with a `stage` discriminator — `searched` (market, signal, per-corpus counts; never retrieved text), `coverage` (`passed`/`declined`/`skipped`), `drafting` — so the agent sees what was searched and why a decline declined; payload shapes are pinned by `DraftStreamProgressV1` and its tests. POST, not GET, because the panel replays the conversation in the body and fetch-based SSE can send the bearer header EventSource cannot. Refinement uses this endpoint with `turns` + `instruction` — no separate refinement route is needed. |
...
| ~~`GET /v1/tickets/{id}/draft-stream`~~ | — | Superseded: streaming shipped in the MVP as `POST .../drafts/stream` (see above). Stage events shipped 2026-08-05 as the `progress` event on that endpoint. |
