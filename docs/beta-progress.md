...
| 2026-08-03 | **Sources-block fix verified: 0 citation failures in 8 full runs**, against roughly 1 run in 3 before. The one non-clean run failed on the sanctioned-code hole instead, which is a different defect. Verified over enough runs to mean something — this is the failure mode that produced "English-only was fixed" while still failing 8% of the time | investigation |
| 2026-08-03 | **The sanctioned-code hole was in the template, not the corpus.** `order-status-sold-out-nothing-to-offer.md` is an APPROVED_REPLY containing "we would also like to offer you a 10% discount ... use the code ITALY10", and retrieval returns it as the *top* approved reply for a plain "what discount can you give me". The prompt says to follow approved replies "when one fits", which was never going to hold against an offer sitting in the context every time. The goodwill sentence is removed from the template; an agent can still add it deliberately for a genuine sold-out order. ITALY10 now appears in zero customer-facing chunks. Verified: 0 discount failures in 15 runs of class E, against roughly 1 in 7 before | investigation |
| 2026-08-03 | **A prompt rule that names the forbidden thing in detail made it five times more likely.** Before touching the template I added a clause telling the model not to lift "a discount, a code, a refund, a replacement" out of a template, with the sold-out case spelled out as the example. Class E went from 0 failing runs in 10 to **5 in 10**. Reverted, then re-measured at 0 in 10 to confirm the clause was the cause and not the weather. Naming a specific remedy in a prohibition appears to prime it — this is the fourth prompt change today and the first that was actively harmful | investigation |
| 2026-08-03 | **`no_unsourced_numbers` caught internal guidance crossing the customer boundary by translation.** `e-repair-time` says the repair "takes up to about 8 weeks". Published policy says "within 1 week of delivery to us" in seven markets; the internal warranty document says headquarters repair "can take 2 months". The model converted the units, so class A — which looks for internal *wording* — saw nothing. The internal corpus also carries the ITALY10 instruction, making it a live route for the discount defect independently of the template. Tracked as its own defect | investigation |
| 2026-08-03 | **Internal guidance was crossing the customer boundary by translation, and the policy answer was right there.** `e-repair-time` told a customer the repair "takes up to about 8 weeks". Published policy says "we will repair your product within 1 week of delivery to us" — and that chunk was retrieved at **rank 0**, so this was never a retrieval failure. The internal warranty document says headquarters repair "can take 2 months"; the model converted the units and preferred it. Class A saw nothing because class A looks for internal *wording* | investigation |
| 2026-08-03 | **The internal corpus was mostly hazard.** Of 14 chunks, six were "Warranty discounts" carrying ITALY10, 10%, 50% and 80% — the exact material behind every commitment defect found today. The other eight are genuine agent workflow: Odoo, sending a replacement, return labels, repair process. Ingest now honours `retrieval: exclude` in front matter, and that one document opts out: it is how to create a discount in Shopify, not anything a reply should rest on. It stays in the repository for people to read. knowledge-v1 is 394 chunks, internal is 8, and **ITALY10 now appears in zero chunks of any corpus** | investigation |
| 2026-08-03 | Measured both ways before choosing: with internal retrieved, class E failed 2 runs in 15; with `--internal-topk 0`, 0 in 12; with the one document excluded and the other eight still retrieved, **0 in 12**. Excluding the document rather than the corpus keeps class A meaningful — eight internal chunks remain for it to catch a leak of | investigation |
| 2026-08-03 | **Correction: the sources-block fix was one variant of two, and I closed it too early.** The delimiter split across a line break is fixed. But the diagnostic now reports "sources block never emitted, model stopped because stop" — a natural completion with no block at all, in roughly 1-2 market-class runs in 10, victim varying. `must_cite_market` is release-blocking, so this still turns the suite red and still shows a grounded draft as unsourced. Reopened | investigation |
